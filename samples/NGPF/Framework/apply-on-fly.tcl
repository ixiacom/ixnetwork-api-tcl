...
#                                                                              #
# Description:                                                                 #
#    This script intends to demonstrate how settings can be changed on the fly #
# Module:                                                                      #
...
puts "Add MAC"
ixNet add $dev1 ethernet -name testmac1
ixNet add $dev2 ethernet -name testmac2
ixNet commit
...
puts "At this moment no change happened and the ApplyOnTheFlyState is '$status'"

puts "Modify IP Address of $ip1"
set ip1MultivalueAddr [ixNet getAttr $ip1 -address]
ixNet setMultiAttr $ip1MultivalueAddr/counter       \
    -direction      increment                       \
    -start          22.1.1.100                      \
...
puts "Modify the gateway of $ip2"
set ip2MultivalueGw [ixNet getAttr $ip2 -gatewayIp]
ixNet setMultiAttr $ip2MultivalueGw/counter         \
    -direction      increment                       \
    -start          22.1.1.100                      \
...
while {$status != "allowed"} {
    ...
    incr count
    if {$count > 90 } {
        error "Waited for $count sec, '/globals/topology -ApplyOnTheFlyState' still not 'allowed' ..."
    }
}

set status [ixNet getAttr /globals/topology -ApplyOnTheFlyState]
puts "Change has been detected and the ApplyOnTheFlyState is '$status'"

puts "Changing the ip and gateway values on the fly"
ixNet exec applyOnTheFly /globals/topology

set status [ixNet getAttr /globals/topology -ApplyOnTheFlyState]
puts "All the changes were applyed and the ApplyOnTheFlyState is '$status'"
...
puts " "
puts "ixNet help ::ixNet::OBJ-/globals/topology -ApplyOnTheFlyState"
puts "[ixNet help ::ixNet::OBJ-/globals/topology -ApplyOnTheFlyState]"
puts " "
puts "ixNet exec applyOnTheFly /globals/topology"
