...
# 	 -symbolicPathName
################################################################################
set pceInitiateLSPParameters1 $pccGroup1/pceInitiateLSPParameters:1
set pathSetupTypeMv [ixNet getAttribute $pceInitiateLSPParameters1 -pathSetupType]
ixNet add $pathSetupTypeMv "singleValue"
ixNet setMultiAttribute $pathSetupTypeMv/singleValue\
            -value "rsvpte"
ixNet commit
set sourceIpv4AddressMv [ixNet getAttribute $pceInitiateLSPParameters1\
								-srcEndPointIpv4]
...
ixNet commit
set destinationIpv4AddressMv [ixNet getAttribute $pceInitiateLSPParameters1\
								-destEndPointIpv4]
...
ixNet commit
set symbolicPathNameMv [ixNet getAttribute $pceInitiateLSPParameters1 -symbolicPathName]
...
################################################################################
set includeBandwidth [ixNet getAttribute $pceInitiateLSPParameters1 -includeBandwidth]
...
# 9. Trigger Return Delegation from PCE side PCInitiate LSPs                   #
################################################################################
if {([ixNet exec returnDelegation $pceInitiateLSPParameters1 [list 0]] != "::ixNet::OK")} {
    puts "Could not Perform Return Delegation on all LSPs:  $::errorInfo"
    return $::FAILED
}
puts "Performing Return Delegation on PCInitiated LSPS..."
after 15000
