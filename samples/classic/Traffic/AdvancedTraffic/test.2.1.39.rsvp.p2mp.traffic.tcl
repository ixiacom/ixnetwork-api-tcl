#-------------------------------------------------------------------------------------
# Name          : test.2.1.39.rsvp.p2mp.traffic.tcl
# Author        : Magesh Manavalan
# Purpose   : Checking P2MP Traffic Item
# Topology  : Configure RSVP TE  P2MP with DUT as Transit  and bidirectional,
#           5 neighbor pair with vlan , 5 IP Endpoints.
# Steps         :
...
set ::TestPart4 0; # Drill Down Stats Verification
...
proc checkDetailTrafficConfiguration {} {
    ...
    routeMesh fullMesh srcDestMesh oneToOne}

    if {[checkAttributeValue $trafficItem $trafficitem_values_1] == 1} {
        return $error
    }

    if {[checkAttributeValue $trafficItem/tracking {trackBy "trackingenabled0\
         vlanVlanId0 mplsMplsLabelValue0 mplsMplsExp0"}] == 1} {
        return $error
    }

    log "Checking Frame Payload parameters..."
    if {[checkAttributeValue $framePayload {type custom \
                                            customPattern AABBCC \
                                            customRepeat True}] == 1} {
        return $error
    }

    log "Checking Frame Size parameters..."
    if {[checkAttributeValue $frameSize {type fixed fixedSize 100}] == 1} {
        return $error
    }

    log "Checking High Level Stream parameters..."
    if {[checkHighLevelSteamCountForAllTrafficItem 1 {0 5}] == 1} {
        return $error
    }
...
proc checkChassiTrafficConfiguration {chassis card port} {
    ...
    log "FAILURE : Error in getting stream 1 data from chassi for port $port"
        return $error
    }

    if {[vlan get $chassisId $card $port] != 0} {
    log "FAILURE : Error in getting VLAN data from chassi for port $port"
        return $error
    }
    ...
    if {[ixExplorerCheckAttributeValue stream {framesize 100 patternType 5}] == 1} {
        return $error
    }

    log "Checking Stream 1 VLAN Values for port $port"
    if {[ixExplorerCheckAttributeValue vlan {vlanID 100 protocolTagId 33024}] == 1} {
        return $error
    }

    log "Checking Stream 1 IPV4 Values for port $port"
    if {[ixExplorerCheckAttributeValue ip {sourceIpAddr 100.100.100.100 \
    destIpAddr 224.0.0.1}] == 1} {
        return $error
    }
...
proc Action {portData1 portData2} {
    ...
    if {[ixNet exec loadConfig [ixNet readFrom \
     $::pwd/config.2.1.39.rsvp.p2mp.traffic.ixncfg]] != "::ixNet::OK"} {
    ...
    if {$::TestPart0 == 1} {
        ...
        set OSPFStatsList {"Sess. Configured" 5 \
                           "Full State Count" 5 \
                           "Full Nbrs." 5}

        set RSVPStatsList1 {"Ingress LSPs Configured" 25 \
                            "Ingress LSPs Up" 25 \
                            "Up State Count" 25}
        set RSVPStatsList2 {"Egress LSPs Up" 25}
        ...
        if {[checkAllProtocolStats $portList "RSVP Aggregated Statistics" $RSVPStatsList1]} {
            log "FAILURE : RSVP Protocol stats are not matching for port $port1"
            ixNetCleanUp
            return $FAILED
        }
        log "RSVP Protocol stats are matching for port $port1"

        set portList [list [list $chassisIp2 $card2 $port2]]
        if {[checkAllProtocolStats $portList "RSVP Aggregated Statistics" $RSVPStatsList2]} {
            log "FAILURE : RSVP Protocol stats are not matching for port $port2"
            ixNetCleanUp
            return $FAILED
        }
        log "RSVP Protocol stats are matching for port $port2"
    }
    ...
    if {($::TestPart4 == 1)} {
        set count 1
        foreach {drilldown_option} {"VLAN:VLAN-ID" "MPLS:Label Value" "MPLS:Experimental"} {
            set caption "Drilldown_$count"
            set usrFilterSetList [subst {{trafficItemFilterId {{name "Traffic"}}}\
                         {sortFilterIds {{name "$drilldown_option"}}}}]

            # Design View
            if {[designL23TrafficFlowStatisticView $caption $usrFilterSetList] == 1} {
            log "FAILURE : Not able to emulate 'Drilldown Per $drilldown_option Statistics View"
            ixNetCleanUp
            return $FAILED
            }

            log "'Drilldown Per $drilldown_option Statistics View Successfully Emulated for TI"
            log "Check Flow Statistics for $caption ..."
            if {[checkAllTrafficStats $caption ] == 1} {
            log "FAILURE : Not able to retrieve statistics values from $caption"
            ixNetCleanUp
            return $FAILED
            }
            incr count
